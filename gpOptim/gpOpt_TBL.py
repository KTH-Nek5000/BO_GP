...
#>>>>> plots
#////////////////////////////////////////////////////////////////////
def gpOpt1d_postProc(xGP,yGP,plotOpts,nTest=100,kernelType_=kernelType):
    """ 
...
#//////////////////////////////////////////////////////////
def gpOpt2d_postProc(xGP,yGP,bounds,plotOpts,kernelType_=kernelType):
    """ 
    ...
    for i in range(len(parID)):  #param-pair loop
        ...
        gpyPlotter_2Dc(fig,ax,meanPred,covarPred,xGP_,yGP,x1_grid,x2_grid,I,J,plotOpts)
...
def gpyPlotter_1D(meanPred,covarPred,xGP,yGP,xTest_,plotOpts):
    ...
    plt.semilogy(xGP[:-1],yGP[:-1],'o r',markersize=7,label=r'Training Data')
...
#///////////////////////////
def gpyPlotter_2Dc(fig,ax,meanPred,covarPred,x,y,x1TestGrid,x2TestGrid,I,J,plotOpts):
    """ 
    ...
    if "varFlag" in plotOpts.keys():
        ...
        ax.plot(x[:,0],x[:,1],'--ok',markersize=markersize,label='Training Data')
        ax.plot(x[len(y)-1,0],x[len(y)-1,1],'--sr',markersize=markersize+2,label='Training Data')
...
def gpSurface_plot(xList, yList, nData, path2figs="../figs/", Rmin=0, Rmax=0, \
    ...
    if nPar>1:
        ...
        gpOpt2d_postProc(xGP,yGP,bounds,plotOpts)
    elif nPar==1:
        ...
        gpOpt1d_postProc(xGP,yGP,plotOpts)
    else:
